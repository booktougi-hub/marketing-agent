...
import { ALL_SEO_CHECKS } from "@/lib/seo/checkRegistry";
import { getScoreBand, SCORE_BAND_LABEL, type ScoreBand } from "@/lib/seo/checkRegistry";
import type { SeoDimScores, SeoGeoCheckResultMap } from "@/types";
...
// Score display + per-check breakdown for the SEO panel
// (components/apps/app-seo-view.tsx). SCORING.md: "surface top 2 findings
// only; full list behind 'show all'" — this is that full list, collapsed by
// default, separate from the AuditFindingCard-rendered top findings above it.
export function SeoScoreSummary({
  score,
  dimScores,
  checkResults,
  runAt,
      ...
      <CardContent className="flex flex-col gap-4">
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-md border p-3">
            <p className="text-xs text-muted-foreground">Retrievability</p>
            <p className="text-xl font-semibold tabular-nums">{dimScores.retrievability}</p>
          </div>
          <div className="rounded-md border p-3">
            <p className="text-xs text-muted-foreground">Off-page presence</p>
            <p className="text-xl font-semibold tabular-nums">{dimScores.off_page}</p>
          </div>
        </div>
