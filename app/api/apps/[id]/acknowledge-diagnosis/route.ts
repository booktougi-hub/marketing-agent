...
// "This looks right — build my strategy" on the diagnosis screen
// (components/apps/diagnosis-screen.tsx). Acknowledges the diagnosis and
// triggers strategy-generation, which now requires apps.diagnosis to be set
// — see PHASES.md's diagnosis-first pipeline redesign (2026-07-14).
export const PATCH = withErrorHandling(async (
  ...
  { params }: { params: Promise<{ id: string }> }
    ...
    if (app.status !== "diagnosis_ready") {
      throw new ValidationError(ErrorMessages.apps.NOT_DIAGNOSIS_READY, "INVALID_STATE");
    }

    await supabaseAdmin
      .from("apps")
      .update({ diagnosis_status: "acknowledged", status: "strategy_pending" })
      .eq("id", id)
      .eq("workspace_id", workspaceId);
    ...
    try {
      const handle = await tasks.trigger<typeof strategyGeneration>("strategy-generation", {
        ...
        diagnosis_correction: parsed.data.correction ?? null,
      });
      await supabaseAdmin
        .from("apps")
