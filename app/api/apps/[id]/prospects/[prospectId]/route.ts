...
export async function PATCH(
  ...
  try {
    ...
    if (existing.status !== "personalised") {
      return NextResponse.json(
        {
          error: "Only personalised prospects can be approved.",
          code: "INVALID_STATE",
        },
        { status: 422 }
      );
    ...
    if (updateError || !updated) {
      return NextResponse.json(
        { error: "Failed to approve prospect.", code: "SERVER_ERROR" },
        { status: 500 }
      );
    }
    ...
    console.error("PATCH /api/apps/[id]/prospects/[prospectId] error:", error);
    return NextResponse.json(
